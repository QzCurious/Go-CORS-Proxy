...
Managed PAC delivers the currently served Generated PAC URL independently to every member of the fixed Managed PAC Service Set. A failed service retains its previous working PAC setting and produces a service-specific warning; successful services are not rolled back when another service fails. Managed PAC retries failed delivery using the newest served projection, but delivery success neither establishes Traffic Projection Current nor activates a feature.

Traffic Routing Ready is true when the PAC Endpoint and Proxy are serving and fresh observation establishes at least one enabled, seamless-cors-owned Network Service setting whose URL identifies this Gateway Runtime's PAC endpoint. An older delivery-generation query remains valid when its host, port, and owned path identify the current endpoint; a marker-owned URL for a previous runtime does not count, and no browser end-to-end probe is required. Reconciliation and Read-Only Status refresh this observation without polling. Even when every attempted service update fails, an earlier observed working setting can preserve Traffic Routing Ready.
